        ...
        <div>
          <h2
            className={cn("text-lg font-medium", {
              "text-green-500": props.done,
              "text-gray-400": !props.done && !props.active,
          ...
          <p
            className={cn("text-sm", {
              "text-gray-600": !props.done && !props.active,
              "text-primary": props.active,
            })}
          >
            {props.description}
...
function Filler(props: { done: boolean }) {
  return (
    <>
      {props.done ? (
        <div className="w-0.5 bg-green-500 my-2 flex grow rounded-full" />
      ) : (
        <div className="flex flex-col grow overflow-hidden justify-evenly">
          {Array.from({ length: 8 }).map((_, index) => (
            <div
              key={index}
              className="w-0.5 h-0.5 bg-gray-400 rounded-full my-1"
            ></div>
          ))}
        </div>
      )}
...
// TODO: USE SOME FORMATTER WITH AUTOSAVE!!!!!!
    ...
    return (
      <div className="h-5 w-5 mt-1 flex justify-center items-center rounded-full border-2 border-gray-600 bg-gray-600">
        {props.running ? (
          ...
          <Minus strokeWidth={3} color="white" size={20} />
        )}
      </div>
    );
