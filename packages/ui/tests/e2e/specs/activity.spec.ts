    ...
    transactions = [
      {
        ...
        bridgeHash:
          "0x931c10f761c1bdfba85de0e85db0d414efb9744703b4d6db9a9e7f5c970522bc",
        delayedInboxHash:
          "0xff268cba66bb2fa4f57529dad590fc2cc3dccf38b09df5005d9f573e15f40e53",
        timestamp: Date.now(),
      },
    ];
...
describe("/activity/:hash", () => {
  ...
  it("should link to etherscan for tracking delayed inbox tx", () => {
    ...
    // assert
    cy.get(
      `a[href='https://sepolia.etherscan.io/tx/${transactions[0].delayedInboxHash}']`
  ...
  it("should link to arbitrum scanner for tracking delayed l2 tx", () => {
    ...
    // assert
    cy.get(
      `a[href='https://sepolia.arbiscan.io/tx/${transactions[0].bridgeHash}']`
    ...
    cy.get("a[data-test-id='create-reminder-btn']") // Adjust the selector to match your specific button
      ...
      .then((href) => {
        ...
        const differenceInMillis =
          startDate.getTime() - transactions[0].timestamp!;
  ...
  it("should link to arbitrum bridge for claiming", () => {
    ...
    // assert
    cy.get(`a[href='https://bridge.arbitrum.io/']`).should(
      "contain.text",
      "Claim funds"
    );
  });
